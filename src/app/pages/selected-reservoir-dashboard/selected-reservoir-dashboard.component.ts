...
export class SelectedReservoirDashboardComponent implements OnInit {
  ...
  myForm: FormGroup;
  reservoirEveryDayUpdateDto: ReservoirEveryDayUpdateDto =
    {} as ReservoirEveryDayUpdateDto;
  ...
  ngOnInit(): void {
    ...
    if (this.id == null) {
      this.findMaintainerByName(
        this.authUser?.authentication.principal?.username
      );
    } else {
      this.isAdmin = true;
    ...
    this.myForm = new FormGroup({
      ...
      presentDepthOfStorage: new FormControl(
        "",
        Validators.max(this.reservoirFullHeight)
      ),
      presentStorage: new FormControl(
        "",
        Validators.max(this.reservoirCapacity)
      ),
  ...
  convertFt2Meter(valNum) {
    if (valNum > 0) return Number(valNum / 3.2808).toFixed(2) + " .m";
    else "NA";
  }
  ...
  findMaintainerByName(name) {
    ...
    this.userService.findMaintainerByName(name).subscribe((res) => {
      ...
      if (this.user?.reservoirs.length > 0) {
        ...
        this.reservoirEveryDayUpdateDto.reservoirId =
          this.user.reservoirs[0].id;
  ...
  onChagne() {
    ...
    if (this.from != "" && this.to != "") {
      ...
      this.reservoirDetailsList = this.backupReservoirDetailsResponseDto.filter(
        (e) => {
  ...
  fetchChartData(reservoirDetailsList: ReservoirDetailsResponseDto[]) {
    ...
    reservoirDetailsList?.forEach((r) => {
      if (r.date != null) xAxisDateWise.push(r.date);
      else xAxisDateWise.push(new Date(r.createdOn).toDateString());
  ...
  findReservoirById(id) {
    this.reservoirService.findReservoirById(id).subscribe(
      (res: any) => {
        this.reservoir = res;
  ...
  findReservoirDetailsById(id) {
    this.reservoirService
      .getReservoirEveryDayDetails(id)
      .subscribe((res: ReservoirDetailsResponseDto[]) => {
        console.log("reservoir details :", res);
        this.reservoirDetailsList = res.reverse();
        console.log(
          "🚀 ~ file: selected-reservoir-dashboard.component.ts:277 ~ SelectedReservoirDashboardComponent ~ .subscribe ~ this.reservoirDetailsList",
          this.reservoirDetailsList
        );
  ...
  submit() {
    if (this.myForm.valid) {
      if (!this.editMode) {
        ...
        this.reservoirEveryDayUpdateDto = {
          ...this.reservoirEveryDayUpdateDto,
          ...this.myForm.value,
        };
        if (
          this.reservoirEveryDayUpdateDto.reservoirId &&
          this.reservoirEveryDayUpdateDto.userId &&
          this.reservoirEveryDayUpdateDto?.date
        ) {
          console.log(this.reservoirEveryDayUpdateDto);
          this.reservoirService
            .updateReservoirEveryDayDetails(this.reservoirEveryDayUpdateDto)
            .subscribe((res) => {
              console.log(res);
        ...
        console.log(
          "else edit = this.reservoirEveryDayUpdateDto",
          this.reservoirEveryDayUpdateDto
        );
        ...
        if (this.reservoirEveryDayUpdateDto.id) {
          console.log(this.reservoirEveryDayUpdateDto);
          this.reservoirService
            .editEveryDayDetails(this.reservoirEveryDayUpdateDto)
            .subscribe((res) => {
              console.log(res);
              this.modalService.dismissAll();
              this.editMode = false;
      ...
      let newLine = "\r\n";
      let msg =
        "Enterd data is Invalid..please make sure to meet this critiria";
      msg += newLine;
